...
${{ordered_fields: 	/**
	* ${description}
	*/
	public ${type}${array_type_suffix} ${name}${array_name_suffix}; 
}}
...
${{ordered_fields: ${getText} }}
    ...
    public String toString(){
    	return "MAVLINK_MSG_ID_${name} -"+${{ordered_fields:" ${name}:"+${string_prefix}${name}${string_suffix}+}}"";
    }
}
''', m)
    f.close()
...
def generate_one(basename, xml):
    ...
    for m in xml.message:
        ...
        for f in m.fields:
            ...
            if f.array_length != 0:
                f.array_type_suffix = '[]'
                f.array_name_suffix = ' = new %s[%u]' % (mavfmt(f),f.array_length)
                f.array_prefix = '*'
                f.array_tag = '_array'
	...
	}''' % (f.name.title(),f.array_length,f.name,mavfmt(f),f.array_length,f.name,f.name.title(),f.array_length,f.name,f.name)
                else:
                    test_strings = []
